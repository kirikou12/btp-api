drop view if exists project_expense_entry_categories;
drop view if exists project_expense_entries;
...
select
    ...
    coalesce(
        ...
        case
            ...
            when invoice.invoice_type = 'SUPPLY_EXCHANGE' then 'Exchange Invoice'
            else 'Usage Invoice'
        end
    ) as category,
    coalesce(
...
where invoice.project_id is not null
  and invoice.invoice_type in ('DIRECT_EXPENSE', 'DIRECT_USAGE', 'SUPPLY_USAGE', 'SUPPLY_EXCHANGE')
