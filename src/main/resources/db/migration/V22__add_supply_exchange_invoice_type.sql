...
alter table supplier_invoices
    drop constraint if exists chk_supplier_invoices_type;

alter table supplier_invoices
    add constraint chk_supplier_invoices_type
    check (invoice_type in ('SUPPLY', 'SUPPLY_USAGE', 'SUPPLY_RETURN', 'SUPPLY_EXCHANGE', 'DIRECT_USAGE', 'DIRECT_EXPENSE'));
